...
function Search() {
  ...
  useEffect(() => {
    dispatch(searchArticle(query)); // This dispatches the searchArticle action
  }, [query, dispatch]);

  // Update state when query or articles change
  useEffect(() => {
  ...
  // Set the document title based on loading and results
  document.title =
    totalArticles === 0
      ? noFound
      : loading
